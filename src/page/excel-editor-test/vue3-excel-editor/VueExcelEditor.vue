        ...
        >
            ...
            >
              ...
              <td
                ...
                :class="{
                  ...
                  expand:
                    item.grouping && ungroup[item.name + record[item.name]],
                  datepick: item.type == 'date',
                  'sticky-column': item.sticky,
                  hideDuplicate:
                    item.hideDuplicate &&
                    rowPos > 0 &&
                    isSameSinceLeft(p, record, pagingTable[rowPos - 1]),
                }"
                :key="p"
                :style="
                  Object.assign(
                    cellStyle(record, item),
                    renderColumnCellStyle(item, record),
                  )
                "
                @mouseover="cellMouseOver"
                @mousemove="cellMouseMove"
              >
                <template v-if="item.format == 'html'">
                  <span
                    v-html="item.toText(record[item.name], record, item, p)"
                  />
                </template>
                <template v-else>
                  {{ item.toText(record[item.name], record, item, p) }}
                </template>
              </td>
              <td
                v-if="vScroller.buttonHeight < vScroller.height"
          ...
          <tfoot>
            <tr v-show="pagingTable.length && summaryRow">
              <td class="row-summary first-col" :class="{ hide: noNumCol }">
                &nbsp;
              </td>
              <td
    ...
    fields: {
      handler() {
        this.lazy(() => {
          ...
          if (this.remember)
            localStorage[
              window.location.pathname + window.location.hash + '.' + this.token
  ...
  methods: {
        ...
        filterColumnList.forEach((k) => {
          switch (true) {
            case this.columnFilter[k].startsWith('<='):
              filter[k] = {
                type: 1,
                modelValue: this.columnFilter[k].slice(2).trim().toUpperCase(),
              }
              if (this.fields[k].type === 'number')
                filter[k].modelValue = Number(filter[k].modelValue)
              break
            case this.columnFilter[k].startsWith('<>'):
              filter[k] = {
                type: 9,
            ...
            case this.columnFilter[k].startsWith('<'):
              filter[k] = {
                type: 2,
                modelValue: this.columnFilter[k].slice(1).trim().toUpperCase(),
              }
              if (this.fields[k].type === 'number')
                filter[k].modelValue = Number(filter[k].modelValue)
              break
            ...
            case this.columnFilter[k].startsWith('*') &&
              ...
              filter[k] = {
                type: 5,
                modelValue: this.columnFilter[k]
                  .slice(1)
                  .slice(0, -1)
                  .trim()
                  .toUpperCase(),
            ...
            case this.columnFilter[k].startsWith('~'):
              filter[k] = {
                type: 8,
                modelValue: this.columnFilter[k].slice(1).trim(),
              }
              break
            ...
            default:
              filter[k] = {
                type: 5,
                modelValue: this.columnFilter[k].trim().toUpperCase(),
              }
              break
          }
        })
        this.filteredValue = this.modelValue.filter((record) =>
          this.recordFilter(record),
        )
        this.filteredValue = this.filteredValue.filter((record, i) =>
          this.filterGrouping(record, i, this.modelValue),
        )
            ...
            */
            ...
            for (let i = 0; i < filterColumnList.length; i++) {
              ...
              switch (filter[k].type) {
                case 0:
                  if (`${content[k]}` !== `${filter[k].modelValue}`)
                    return false
                  break
                case 1:
                ...
                case 7:
                  ...
                  break
                case 8:
                  if (!new RegExp(filter[k].modelValue, 'i').test(content[k]))
                    return false
                  break
                case 9:
                  if (`${content[k]}` === `${filter[k].modelValue}`)
      ...
      this.fields.forEach((field) => {
        ...
        if (field.grouping && rec[name] === prec[name]) {
          if (
            ...
            this.ungroup[field.name + rec[name]] !== true
          )
            result = false
          else if (
            field.grouping === 'expand' &&
            this.ungroup[field.name + rec[name]]
          )
            result = false
        }
      })
    ...
    renderColumnCellStyle(field, record) {
      ...
      if (record && field.color)
        result.color =
          typeof field.color === 'function' ? field.color(record) : field.color
      return result
    },
    localeDate(d) {
    ...
    columnFillWidth() {
      ...
      this.lazy(() => {
        ...
        let viewWidth = this.fields
          .filter((f) => !f.invisible)
          .reduce((c, f) => c + parseFloat(f.width), 0)
        ...
        if (Math.abs(fillWidth) > 1)
          doFields.forEach((f) => {
            ...
            f.width =
              (w > parseFloat(f.origWidth) ? w : parseFloat(f.origWidth)) + 'px'
          })
      })
    },
    ...
    datepickerClick() {
      ...
      switch (this.currentField.type) {
        case 'date':
          ...
          this.inputBox.value = new Date(new Date(this.inputDateTime) - offset)
            .toISOString()
            .slice(0, 10)
          break
        case 'datetime':
          ...
          this.inputBox.value =
            new Date(new Date(this.inputDateTime) - offset)
              .toISOString()
              .replace('T', ' ')
              .slice(0, 16) + ':00'
          break
        ...
        case 'datetimetick':
        case 'datetimesectick':
          this.inputBox.value = new Date(
            new Date(this.inputDateTime) - offset,
          ).getTime()
          break
      }
      this.inputBoxShow = 0
    ...
    ftMouseDown(e) {
      ...
      const ratio =
        (e.x - footerRect.left - this.numColWidth) /
        (footerRect.width - this.numColWidth)
    ...
    sbMouseDown(e) {
      ...
      if (!this.hScroller.mouseX) {
        ...
        const fleft =
          this.footer.getBoundingClientRect().left + this.numColWidth
    ...
    mousewheel(e) {
      ...
      let adjust = 0
      if (
        e.deltaY > 1 * this.wheelSensitivity &&
        this.pageTop + this.pageSize < this.table.length
      )
        adjust = 1
      else if (e.deltaY < -1 * this.wheelSensitivity && this.pageTop > 0)
        adjust = -1
      if (adjust) {
        this.pageTop += adjust
      ...
      if (this.inputBoxShow && this.currentField.type === 'password') {
        setTimeout(() => {
          const v = this.inputBox.value
            .split('')
            .map((c, i) =>
              c === this.currentField.masking ? this.inputBox._value[i] : c,
            )
      ...
      if (e.ctrlKey || e.metaKey)
        ...
        switch (e.keyCode) {
          ...
          default:
            ...
            if (!this.inputBoxShow) {
              if (
                ['select', 'map', 'action'].includes(this.currentField.type)
              ) {
                setTimeout(() => this.calAutocompleteList(true))
    ...
    headerClick(e, colPos) {
      ...
      if (e.which === 1) {
        ...
        else if (this.sortDir === 0) this.sort(1, colPos)
        else this.sort(0, colPos)
      }
    },
    ...
    refreshPageSize() {
      if (this.$refs.hScroll) {
        ...
        this.hScroller.scrollerUnseenWidth =
          this.footer.getBoundingClientRect().width -
          this.numColWidth -
          scrollerWidth
      }
    ...
    exportTable(format, selectedOnly, filename) {
      ...
      setTimeout(() => {
        ...
        let ws1 = null
        let data = this.table
        if (selectedOnly) data = this.table.filter((rec, i) => this.selected[i])
    ...
    */
      ...
      if (e.shiftKey) {
        ...
        if (this.prevSelect !== -1 && this.prevSelect !== rowPos) {
          ...
          if (rowPos > this.prevSelect)
            for (let i = this.prevSelect; i <= rowPos; i++) this.selectRecord(i)
          else
            for (let i = rowPos; i <= this.prevSelect; i++) this.selectRecord(i)
        }
      } else {
    ...
    moveSouth() {
      if (this.focused) {
        ...
        if (
          this.currentRowPos + 1 >= this.pageBottom - this.pageTop &&
          this.pageBottom >= this.table.length
        ) {
    ...
    mouseDown(e) {
      if (
        ...
        if (
          this.currentField &&
          this.currentField.link /* && e.altKey */ &&
          this.currentCell.textContent
        )
          return setTimeout(() =>
        ...
        if (this.currentField.grouping) {
          this.ungroup[this.currentField.name + this.currentCell.textContent] =
            !this.ungroup[this.currentField.name + this.currentCell.textContent]
          this.refresh()
          return
        }
        ...
        if (e.target.offsetWidth < e.target.scrollWidth) {
          ...
          setTimeout(() => {
            ...
            else this.$refs.texttip.style.left = rect.left + 'px'
    ...
    cellMouseMove(e) {
      ...
      if (e.target.offsetWidth - e.offsetX < 25) {
        if (
          ...
          cursor = 'pointer'
        if (e.target.offsetWidth < e.target.scrollWidth) cursor = 'pointer'
      }
      const row = e.target.parentNode
      const colPos = Array.from(row.children).indexOf(e.target) - 1
      ...
      transaction.every((t) => {
        try {
          if (t.type === 'd') {
            ...
            const valueRowPos = this.modelValue.findIndex(
              (v) => v.$id === t.$id,
            )
            if (valueRowPos >= 0) {
    ...
    newRecord(rec, selectAfterDone, noLastPage, isUndo) {
      ...
      this.fields.map((f) => {
        if (typeof rec[f.name] === 'undefined') {
          if (f.keyField) rec[f.name] = '§' + this.tempKey()
      ...
      setTimeout(() => {
        Object.keys(this.selected).forEach((recPos) => {
          ...
          this.updateCell(
            pos,
            field,
            field.toValue(setText, this.table[pos], field),
          )
        })
        this.processing = false
      }, 0)
        ...
        this.table.map((row) =>
          Promise.all(
            Object.keys(row).map((name) =>
              this.updateCell(row, name, row[name], false),
            ),
          ),
        ),
      )
      ...
      if (force || (this.inputBoxChanged && this.inputBox.value.length > 0)) {
        ...
        const doList = async () => {
          ...
          this.autocompleteSelect = list.findIndex((element) =>
            element?.toString().toUpperCase().startsWith(value),
          )
